...
CUT='2026-07-31'
chunks=json.load(open(P+'rescue_leria_chunks.json'))
try: state=json.load(open(P+'rescue_leria_state.json'))
except Exception: state={'done':[],'invoices':[]}
log=open(P+'rescue_leria.log','a',buffering=1)
DRAFT=[['company_id','=',1],['move_type','=','out_invoice'],['state','=','draft'],['partner_id','=',6]]
for idx,ch in enumerate(chunks):
    ...
    json.dump(state,open(P+'rescue_leria_state.json','w'),ensure_ascii=False,default=str)
